...
namespace CitrixAdminTool.Wpf.Services.Localization
{
    ...
    internal static class StringTable
    {
        ...
        private static readonly Dictionary<string, Entry> Table = new Dictionary<string, Entry>
        {
            ...
            // デリバリーグループ一覧
            // ============================================================
            { "Main_DesktopGroups",    E("デリバリーグループ", "Delivery groups") },
            { "Dg_Title",              E("デリバリーグループ - {0}", "Delivery groups - {0}") },
            { "Dg_ListName",           E("デリバリーグループ一覧", "Delivery group list") },
            { "Dg_HintReady",          E("「更新」でデリバリーグループ一覧を取得します。", "Click \"Refresh\" to load the delivery groups.") },
            { "Dg_HintCredential",     E("「更新」を押すと資格情報の入力を求めます。", "Clicking \"Refresh\" will prompt for credentials.") },
            { "Dg_Loading",            E("デリバリーグループを取得しています...", "Loading delivery groups...") },
            { "Dg_Loaded",             E("{0} 件のデリバリーグループを取得しました。", "Loaded {0} delivery group(s).") },
            { "Dg_SummaryAll",         E("全 {0} 件", "{0} group(s)") },
            { "Dg_SummaryFiltered",    E("表示 {0} 件 / 全 {1} 件", "Showing {0} of {1} group(s)") },
            { "Dg_LabelKind",          E("種別:", "Kind:") },
            { "Dg_LabelDelivery",      E("配信:", "Delivery:") },
            { "Dg_LabelEnabled",       E("有効:", "Enabled:") },
            { "Dg_FilterEnabled",      E("有効のみ", "Enabled only") },
            { "Dg_FilterDisabled",     E("無効のみ", "Disabled only") },
            { "Dg_TipKind",            E("Private（専有）／Shared（共有）で絞り込みます。", "Filter by Private (dedicated) or Shared.") },
            { "Dg_TipDelivery",        E("配信形態（デスクトップのみ／アプリのみ／両方）で絞り込みます。", "Filter by delivery type (desktops only / apps only / both).") },
            { "Dg_TipSearch",          E("名前・表示名・説明の部分一致で検索します（大文字小文字は区別しません）。", "Substring search over name, published name and description (case-insensitive).") },
            { "Dg_ColName",            E("名前", "Name") },
            { "Dg_ColPublishedName",   E("表示名", "Published name") },
            { "Dg_ColEnabled",         E("有効", "Enabled") },
            { "Dg_ColKind",            E("種別", "Kind") },
            { "Dg_ColDelivery",        E("配信", "Delivery") },
            { "Dg_ColTotal",           E("マシン数", "Machines") },
            { "Dg_ColAvailable",       E("利用可", "Available") },
            { "Dg_ColInUse",           E("使用中", "In use") },
            { "Dg_ColUnregistered",    E("未登録", "Unregistered") },
            { "Dg_ColDescription",     E("説明", "Description") },
            { "Dg_CsvSaved",           E("{0} 件をCSVに保存しました: {1}", "Saved {0} group(s) to CSV: {1}") },
            { "Dg_Copied",             E("{0} 件をクリップボードにコピーしました（Excelに貼り付けられます）。", "Copied {0} group(s) to the clipboard (ready to paste into Excel).") },
            { "Dg_CsvDialogTitle",     E("デリバリーグループ一覧をCSVに保存", "Save delivery group list as CSV") },
            { "Op_ListDesktopGroups",  E("デリバリーグループ一覧取得（Get-BrokerDesktopGroup）", "List delivery groups (Get-BrokerDesktopGroup)") },
            { "Log_DesktopGroupTable", E("デリバリーグループ一覧（{0} 件）", "Delivery groups ({0})") },
            ...
            { "Machines_TipPowerMenu", E("選択したマシンに対して電源操作を行います。\nそのマシンで作業中の利用者はすべて切断されます。",
                                        "Performs a power operation on the selected machines.\nEveryone working on those machines will be disconnected.") },
            { "Machines_PowerAffected", E("⚠ これらのマシンでは合計 {0} 件のセッションが動作しています。対象の利用者はすべて切断されます。",
                                        "WARNING: these machines are running {0} session(s) in total. Every one of those users will be disconnected.") },
            { "Machines_PowerNoSessions", E("これらのマシンで動作中のセッションはありません（前回の「更新」時点）。",
                                        "No sessions were running on these machines as of the last refresh.") },
            ...
            { "Lic_Header",            E("ライセンス:", "License:") },
            { "Lic_Server",            E("サーバー", "Server") },
            { "Lic_Product",           E("製品", "Product") },
            { "Lic_GraceActive",       E("猶予期間中（残り {0} 時間）— ライセンスサーバーと通信できていません。猶予が切れると新規セッションが拒否されます",
                                        "GRACE PERIOD ACTIVE ({0} hours left) - the license server cannot be reached. New sessions will be refused when the grace period expires") },
            { "Lic_WarnShort",         E("⚠ ライセンス要確認", "⚠ Check licensing") },
            { "Log_AuthIntegratedAs",  E("統合Windows認証（{0}）", "Integrated Windows authentication ({0})") },
            { "Log_AuthCredentialNote", E("↑ このアカウントのネットワークIDで起動した専用ワーカーで接続しました。",
                                        "Connected through a dedicated worker started with this account's network identity.") },
            { "Log_Summary",           E("集計: {0} サイト中 {1} サイト成功 / {2} サイト失敗",
                                        "Summary: {1} of {0} site(s) succeeded / {2} failed") },
            { "Log_Site",              E("サイト", "Site") },
